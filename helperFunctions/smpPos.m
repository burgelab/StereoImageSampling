function posDeg = smpPos(smpPerUnit,numSmp,bStartAtZero)

% function posDeg = smpPos(smpPerUnit,numSmp,bStartAtZero)
%
%   example call: posDeg = smpPos(128,128)
% 
% spatial positions of samples given a sampling rate and a patch
% spatial positions are always sampled at zero
%
% smpPerUnit:   sampling rate (e.g. smp/deg, smp/sec, etc)
...
%%%%%%%%%%%
% posDeg:          spatial positions of sample
%
%                      *** see smpFrq.m ***
...
elseif mod(numSmp,2) == 1
    minPos = -0.5*(numSmp-1)./smpPerUnit;
    maxPos =  0.5*(numSmp-1)./smpPerUnit;
else
    error(['smpPos: WARNING! num pix must be integer valued. numSmp = ' num2str(numSmp)]);
end
posDeg    = linspace(minPos,maxPos,max(numSmp));

if bStartAtZero == 1
   posDeg = posDeg-minPos; 
end