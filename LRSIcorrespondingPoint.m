...
%function [LitpRC,RitpRC,CitpRC,LitpRCchk,RitpRCchk] =  LRSIcorrespondingPoint(LorR,IctrRC,Lpht,Rpht,Lxyz,Rxyz,LppXm,LppYm,RppXm,RppYm,CppZm,IPDm,bPLOT,wallORcross)
%
% Example call: load('LRSItestImg02.mat'); load('LRSIprojPlaneData.mat'); [LitpRC,RitpRC] = LRSIcorrespondingPoint('R',[805 1160],Limg,Rimg,Lxyz,Rxyz,LppXm,LppYm,RppXm,RppYm,CppZm,IPDm,bPLOT,wallORcross);
%
% Yields corresponding points in left- and right-eye images given a candidate patch-center in one of the eye's (anchor eye's) image
%
% LorR:             anchor eye (i.e. eye from whose image the interpolation procedure is initiated)
...
if ~exist('bTest','var')        || isempty(bTest)         bTest       = 0;      end
...
if     strcmp(LorR,'L')
        ...
        try [RitpRCchk,LitpRCchk ] = LRSIcorrespondingPointR2L(round(RitpRC),Rxyz,Lxyz,RppXm,RppYm,CppZm,IPDm,0);
        catch, LitpRCchk = [NaN NaN]; RitpRCchk = [NaN NaN];
        end
    elseif strcmp(LorR,'R')
        ...
        try [LitpRCchk,RitpRCchk ] = LRSIcorrespondingPointL2R(round(LitpRC),Lxyz,Rxyz,LppXm,LppYm,CppZm,IPDm,0);
        catch, LitpRCchk = [NaN NaN]; RitpRCchk = [NaN NaN];
        end
